//! v0.51.1 Slice 3 durability (MinIO/TC): a bare `INSERT` with no
//! `RETURNING`, no explicit `BEGIN`, and no `SET` autocommits and must
//! survive a process restart against the same SlateDB-on-S3 (MinIO) backend.
...
const MINIO_BUCKET: &str = "rockstream-autocommit-durability-test";
...
#[tokio::test]
async fn bare_insert_persists_across_reconnect_minio() {
    if !docker_available() {
        eprintln!("SKIP bare_insert_persists_across_reconnect_minio: Docker not available");
        return;
    }
    ...
    run_bare_insert_restart_case(minio_object_store(port), "autocommit-durability-minio").await;
}
