//! v0.51.1 Slice 1 durability (MinIO/TC): positionally-resolved values from a no-column-list multi-row INSERT must be correctly persisted (not NULL phantom rows) against the same SlateDB-on-S3 (MinIO) backend.
...
const MINIO_BUCKET: &str = "rockstream-insert-no-column-list-durability-test";
...
async fn run_positional_values_restart_case(store: Arc<dyn ObjectStore>, shard_path: &str) {
    ...
    client
        ...
        .unwrap();
    client
        .simple_query("SET rockstream.idempotency_key = 'no-cols-seed'")
        .await
        .unwrap();
    client
    ...
    client
        .simple_query("SET rockstream.idempotency_key = 'no-cols-append'")
        .await
        .unwrap();
    client
        .simple_query("INSERT INTO t VALUES (4, 'dave')")
        .await
        .unwrap();
    client.simple_query("COMMIT").await.unwrap();
...
#[tokio::test]
async fn positional_values_persisted_minio() {
    let (_container, port) = match rockstream_test_support::minio::start_minio(MINIO_BUCKET).await {
        Some(m) => m,
        None => {
            eprintln!("SKIP positional_values_persisted_minio: Docker not available");
            return;
        }
    };
