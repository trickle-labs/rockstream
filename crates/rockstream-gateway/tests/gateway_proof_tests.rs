...
// ══════════════════════════════════════════════════════════════════════════════
// v0.44 slice 10: EXPLAIN sink-target annotation (P4)
// ══════════════════════════════════════════════════════════════════════════════

/// EXPLAIN over a view with a registered `CREATE SINK` shows a `sink_target:`
/// annotation naming the sink, its format, path, and state.
#[tokio::test]
async fn explain_shows_sink_target_for_sunk_view_via_wire() {
    let catalog = Arc::new(CatalogStubs::new());
    let server = GatewayServer::with_catalog(
        "127.0.0.1:0".parse().unwrap(),
        catalog.clone(),
    ...
    client
        .simple_query("CREATE VIEW sunk_view AS SELECT id, val FROM base WHERE val > 0")
        .await
        .expect("CREATE VIEW failed");

    client
        .simple_query(
            "CREATE SINK sunk_view_sink FOR VIEW sunk_view TO ICEBERG 'file:///warehouse/sunk_view' WITH (catalog=filesystem)",
        )
        .await
        .expect("CREATE SINK failed");

    let msgs = client
        .simple_query("EXPLAIN SELECT * FROM sunk_view")
        .await
        .expect("EXPLAIN failed");
    ...
    assert!(
        plan_output.contains("sink_target: name='sunk_view_sink'"),
        "EXPLAIN output must contain sink_target for a sunk view; got:\n{plan_output}"
    );
    assert!(
        plan_output.contains("format=ICEBERG"),
        "EXPLAIN sink_target must show format=ICEBERG; got:\n{plan_output}"
    );
    assert!(
        plan_output.contains("path='file:///warehouse/sunk_view'"),
        "EXPLAIN sink_target must show the sink path; got:\n{plan_output}"
    );
    assert!(
        plan_output.contains("state=OK"),
        "EXPLAIN sink_target must show state=OK when no catalog-registration warning is set; got:\n{plan_output}"
    );
}
