//! v0.51.1 Slice 2 durability (MinIO/TC): a server-generated idempotency key must be persisted and survive a process restart against the same SlateDB-on-S3 (MinIO) backend.
...
const MINIO_BUCKET: &str = "rockstream-idempotency-envelope-durability-test";
...
async fn run_autogenerated_key_restart_case(store: Arc<dyn ObjectStore>, shard_path: &str) {
    ...
    client
        ...
        .await
        .unwrap();
    // No SET rockstream.idempotency_key / source_epoch — server mints one.
    client
...
#[tokio::test]
async fn autogenerated_key_persisted_minio() {
    if !docker_available() {
        eprintln!("SKIP autogenerated_key_persisted_minio: Docker not available");
        return;
    }
    ...
    run_autogenerated_key_restart_case(minio_object_store(port), "idempotency-envelope-durability-minio").await;
}
