//! v0.51.1 Slice 2 durability (LFS): a server-generated idempotency key
//! (no explicit `SET rockstream.idempotency_key`/`source_epoch`) must be
//! persisted via `put_idempotency_key` and survive a process restart against
//! the same SlateDB-on-LocalFileSystem backend, so a replayed commit after
//! recovery is still recognized.
...
#[tokio::test]
async fn autogenerated_key_persisted_lfs() {
    let dir = tempfile::tempdir().unwrap();
    let store: Arc<dyn ObjectStore> =
        Arc::new(LocalFileSystem::new_with_prefix(dir.path()).unwrap());
    let catalog = Arc::new(CatalogStubs::new());
    let shard_path = "idempotency-envelope-durability-lfs";
