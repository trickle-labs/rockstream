...
use rockstream_connectors::{DeltaSink, FaultInjectingObjectStore, SinkConnector, SinkError};
...
async fn run_delta_recovery_scenario(store: Arc<dyn ObjectStore>, base_path: &str) {
    ...
    let mut sink = DeltaSink::new(ConnectorId(244), store, base_path);
...
async fn test_deltalake_sink_recovery_lfs() {
    let dir = TempDir::new().unwrap();
    let store: Arc<dyn ObjectStore> =
        Arc::new(LocalFileSystem::new_with_prefix(dir.path()).unwrap());
    run_delta_recovery_scenario(store, "delta-lfs").await;
}

#[tokio::test(flavor = "multi_thread")]
async fn test_deltalake_sink_recovery_minio_tc() {
    if !docker_available() {
        eprintln!("SKIP test_deltalake_sink_recovery_minio_tc: Docker not available");
        return;
    }

    let container = MinIO::default().start().await.unwrap();
    let port = container.get_host_port_ipv4(9000).await.unwrap();
    create_minio_bucket(port, "delta-recovery-test").await;
    let store = build_minio_store(port, "delta-recovery-test");
    run_delta_recovery_scenario(store, "delta-minio").await;
}
