...
async fn test_nexmark_q0_q9_minio() {
    ...
    let (_minio, port) = match start_minio("testbucket").await {
        Some(m) => m,
        None => {
            eprintln!("Docker is not available, skipping MinIO test");
            return;
        }
    };

    let store = Arc::new(minio_object_store(port, "testbucket"));
