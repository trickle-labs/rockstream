...
pub struct EpochCompactor {
    ...
    active_epochs: BTreeMap<Epoch, ActiveEpochState>,
}
...
impl EpochCompactor {
    ...
    pub fn open_epoch(&mut self, epoch: Epoch) -> &mut ActiveEpochState {
        let now = Instant::now();
        self.active_epochs
            .entry(epoch)
    ...
    pub fn push_row(&mut self, epoch: Epoch, row: RuntimeRow) {
        self.push_row_at(epoch, row, Instant::now());
    }

    /// Ingest a single row delta specifying timestamp explicitly.
    pub fn push_row_at(&mut self, epoch: Epoch, row: RuntimeRow, now: Instant) {
    ...
    pub fn push_rows<I>(&mut self, epoch: Epoch, rows: I)
    where
    ...
    pub fn push_delta(&mut self, epoch: Epoch, values_tsv: impl Into<String>, weight: i64) {
        self.push_row(
            epoch,
            RuntimeRow {
    ...
    pub fn push_deltas<I, S>(&mut self, epoch: Epoch, deltas: I)
    where
    ...
    pub fn drain_epoch(&mut self, epoch: Epoch) -> Option<Vec<RuntimeRow>> {
        let state = self.active_epochs.remove(&epoch)?;
    ...
    pub fn flush_epoch(&mut self, epoch: Epoch) -> Option<Vec<RuntimeRow>> {
        self.drain_epoch(epoch)
    }

    /// Drain all expired active epochs.
    pub fn drain_expired_epochs(&mut self) -> Vec<(Epoch, Vec<RuntimeRow>)> {
        self.drain_expired_epochs_at(Instant::now())
    }

    /// Drain all expired active epochs relative to `now`.
    pub fn drain_expired_epochs_at(&mut self, now: Instant) -> Vec<(Epoch, Vec<RuntimeRow>)> {
    ...
    pub fn flush_expired_epochs(&mut self) -> Vec<(Epoch, Vec<RuntimeRow>)> {
        self.drain_expired_epochs()
    }

    /// Drain all active epochs regardless of window expiration.
    pub fn drain_all(&mut self) -> Vec<(Epoch, Vec<RuntimeRow>)> {
    ...
    pub fn flush_all(&mut self) -> Vec<(Epoch, Vec<RuntimeRow>)> {
        self.drain_all()
    }
        ...
        I: IntoIterator<Item = RuntimeRow>,
    {
        ...
        let mut non_zero: Vec<InFlightRow> = map
            .into_values()
            .filter(|r| r.weight != 0)
            .collect();
...
mod tests {
    ...
    fn test_zero_net_weight_omission() {
        let mut compactor = EpochCompactor::default();
        let epoch = 1;
    ...
    fn test_primary_key_matching_collapse() {
        ...
        let mut compactor = EpochCompactor::new(config);
        let epoch = 10;
