...
//! down on exit.
//!
//! Uses `MinIO2024` which pins the MinIO server image and configures the command,
//! wait condition, and exposed ports.
...
use rockstream_test_support::minio::create_minio_bucket;
...
async fn get_shared_minio() -> Option<u16> {
    let res = SHARED_MINIO
        .get_or_init(|| async { rockstream_test_support::minio::start_minio(MINIO_BUCKET).await })
        .await;
