...
fn test_workspace_version_matches_candidate_identity() {
    ...
    assert_eq!(id.semantic_version, "0.59.4");
    ...
    assert!(
        cargo_toml.contains("version = \"0.59.4\""),
        "Workspace Cargo.toml must declare version = \"0.59.4\""
    );
}
...
fn test_cli_version_structured_identity() {
    ...
    assert!(text.contains("rockstream 0.59.4"));
    ...
    assert_eq!(v["semantic_version"], "0.59.4");
...
fn test_dockerfile_image_labels_match_identity() {
    ...
    assert!(
        dockerfile.contains("org.opencontainers.image.version=\"0.59.4\""),
...
fn test_manifest_docs_conformance() {
    ...
    assert!(
        cap_toml.contains("version = \"v0.59.4\""),
        "capabilities.toml contract version must be v0.59.4"
    );
}
