...
fn test_workspace_version_matches_candidate_identity() {
    ...
    assert!(
        cargo_toml.contains("version = \"0.59.7\""),
        "Workspace Cargo.toml must declare version = \"0.59.7\""
    );
}
...
fn test_cli_version_structured_identity() {
    ...
    assert!(text.contains("rockstream 0.59.7"));
    ...
    assert_eq!(v["semantic_version"], "0.59.7");
