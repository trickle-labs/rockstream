...
async fn drain(reader: &mut BufReader<TcpStream>, worker_id: WorkerId) {
    ...
    while !(begin_seen && status_seen) {
        ...
        tokio::time::timeout(Duration::from_secs(1), reader.read_line(&mut response))
            ...
            .unwrap();
        match serde_json::from_str::<ControlMessage>(response.trim()).unwrap() {
            ControlMessage::BeginDrain(_) => begin_seen = true,
            ControlMessage::DrainStatus { .. } => status_seen = true,
            _ => {}
        }
    }
...
async fn drain_converges_under_buggify_seed() {
    ...
    drain(&mut worker_streams[0], WorkerId(1)).await;
