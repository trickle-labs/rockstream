...
- Multi-tenancy and secrets: `CREATE SECRET`/envelope encryption/KEK configuration are v0.56 scope; `ALTER SCHEMA/NAMESPACE ... SET DEFAULT WORKLOAD` (schema/namespace-level default workload assignment, distinct from the per-view `WITH WORKLOAD = '<name>'` assignment clause, which is implemented) remains unimplemented; none exist in `rockstream-sql`/`rockstream-gateway` today.
