...
- Optimistic transactions: `SERIALIZABLE LOCAL`, exact-key guarded writes, optimistic conflict detection (`RS-2008`), and a CRDT-only transaction envelope are v0.54 scope, planned behind `--experimental-optimistic-crdt-transactions`.
