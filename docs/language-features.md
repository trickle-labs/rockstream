...
- **Tier: Core** — Exact integer and decimal aggregation: exact summation and counts across integer and decimal types. Proof: [`core_aggregate_exact_int_sum`](../crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs).
- **Tier: Maintain** — Text aggregation: `MIN` and `MAX` over string types. Proof: [`core_aggregate_text_min_max`](../crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs).
- **Tier: Experimental** — Approximate floating-point aggregation: `SUM`, `AVG`, `MIN`, `MAX` over floating-point values. Proof: [`core_aggregate_float_sum`](../crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs).
- **Tier: Core** — Exact integer key equi-joins: inner, left, right, and full outer joins over integer keys. Proof: [`core_join_equi_integer`](../crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs).
- **Tier: Maintain** — Binary UTF-8 text key equi-joins: joins over UTF-8 text keys under `rockstream_binary_v1` collation. Proof: [`core_join_equi_text`](../crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs).
- **Tier: Experimental** — Cartesian cross-joins: cross joins without join keys. Proof: [`core_join_cross`](../crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs).
- **Tier: Experimental** — Floating-point key joins: floating-point join keys are unsupported and rejected fail-closed with `RS-1019`. Proof: [`core_join_inner_f64`](../crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs).
- **Tier: Maintain** — Ranking functions: `ROW_NUMBER`, `RANK`, `DENSE_RANK`, `LAG`, `LEAD` window analytics. Proof: [`core_window_ranking`](../crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs).
- **Tier: Core** — Fixed tumbling time windows: non-overlapping tumble time window aggregation. Proof: [`compiled_tumble_window_state_persists_across_restart_lfs`](../crates/rockstream-gateway/tests/live_exec_tumble_window_durability_lfs_tests.rs).
- **Tier: Core** — Sliding hopping time windows: overlapping sliding window aggregation. Proof: [`core_window_sliding`](../crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs).
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `connector.kafka-source` | `incremental` | A Kafka record is polled and emitted as an exact incremental delta batch. | `crates/rockstream-connectors/tests/kafka_source_guarantee_matrix_tests.rs::kafka_source_incremental_stream_has_exact_transcript` | — | — | — | — |
| `connector.kafka-source` | `backfill` | A Kafka source starting at earliest offset backfills all partition records before subsequent poll deltas. | `crates/rockstream-connectors/tests/kafka_source_guarantee_matrix_tests.rs::kafka_source_earliest_backfill_all_records` | — | — | — | — |
| `connector.kafka-source` | `checkpoint_recovery` | A recovered source resumes at the committed offset and observes exactly the records produced after it in both local and object-store backends. | `crates/rockstream-connectors/tests/kafka_source_guarantee_matrix_tests.rs::kafka_source_committed_offset_recovery_has_exact_transcript` | `crates/rockstream-connectors/tests/source_durability_minio_tests.rs::retained_source_checkpoint_recovery_has_exact_cdc_and_kafka_transcript_minio` | — | — | — |
| `connector.kafka-source` | `state_growth` | Kafka source buffer is bounded by the configured credit limit and exposes its fill metric. | `crates/rockstream-connectors/tests/dlq_backpressure_threshold_tests.rs::kafka_source_queue_bounded_with_fill_metric` | — | configured poll credit limit | kafka_source_poll_fill_level | poll applies backpressure or returns its RS-coded resource error |
| `connector.kafka-source` | `failure` | An unreachable broker or invalid configuration fails closed with an actionable RS-coded error. | `crates/rockstream-connectors/tests/kafka_source_non_broker_tests.rs::kafka_source_unreachable_broker_fails_closed` | — | — | — | — |
| `connector.postgres-cdc` | `incremental` | A PostgreSQL logical replication delta is captured and emitted as an exact incremental record batch. | `crates/rockstream-connectors/tests/postgres_cdc_guarantee_matrix_tests.rs::postgres_cdc_snapshot_stream_fence_has_exact_transcript` | — | — | — | — |
| `connector.postgres-cdc` | `backfill` | Initial snapshot backfills every committed row from the source PostgreSQL table before streaming live deltas. | `crates/rockstream-connectors/tests/postgres_cdc_guarantee_matrix_tests.rs::postgres_cdc_initial_snapshot_backfills_all_rows` | — | — | — | — |
| `connector.postgres-cdc` | `checkpoint_recovery` | Each committed replication-slot LSN boundary recovers exactly the changes committed after it in both local and object-store backends. | `crates/rockstream-connectors/tests/postgres_cdc_guarantee_matrix_tests.rs::postgres_cdc_each_commit_boundary_recovers_exactly_once` | `crates/rockstream-connectors/tests/cdc_transaction_durability_minio_tests.rs::shared_slot_checkpoint_reopens_once_minio` | — | — | — |
| `connector.postgres-cdc` | `state_growth` | PostgreSQL CDC queue is bounded by the configured in-flight limit and exposes its fill metric. | `crates/rockstream-connectors/tests/dlq_backpressure_threshold_tests.rs::postgres_cdc_queue_bounded_with_fill_metric` | — | configured in-flight record queue | postgres_cdc_queue_fill_ratio | the source applies backpressure or rejects with its RS-coded resource error |
| `connector.postgres-cdc` | `failure` | An invalid or lost replication slot fails closed with an actionable RS-coded error. | `crates/rockstream-connectors/tests/postgres_cdc_guarantee_matrix_tests.rs::postgres_cdc_invalid_slot_fails_closed` | — | — | — | — |
| `language.aggregates.exact` | `incremental` | Exact aggregation deltas update the grouped sum and count transcripts incrementally. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_aggregate_exact_incremental` | — | — | — | — |
| `language.aggregates.exact` | `backfill` | Aggregate backfill evaluates the complete snapshot over exact numeric groups before deltas. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_aggregate_exact_backfill` | — | — | — | — |
| `language.aggregates.exact` | `checkpoint_recovery` | A reopened aggregate shard restores the committed exact grouped results. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_aggregate_exact_checkpoint_recovery` | — | — | — | — |
| `language.aggregates.exact` | `state_growth` | Exact aggregate arrangements use the configured state budget and expose fill level. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_aggregate_exact_state_growth` | — | configured aggregate state budget | aggregate_state_fill_ratio | the write is rejected with its RS-coded resource error |
| `language.aggregates.exact` | `failure` | Invalid aggregate expressions fail closed with an actionable RS-coded error. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_aggregate_exact_failure` | — | — | — | — |
...
| `language.joins.equi-integer` | `incremental` | Integer equi-join deltas update the relational result without changing unaffected rows. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_join_equi_integer_incremental` | — | — | — | — |
| `language.joins.equi-integer` | `backfill` | Join backfill builds the complete snapshot result before later input deltas. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_join_equi_integer_backfill` | — | — | — | — |
| `language.joins.equi-integer` | `checkpoint_recovery` | A reopened join shard restores the committed relational transcript in both local and object-store backends. | `crates/rockstream-gateway/tests/live_exec_join_durability_lfs_tests.rs::compiled_join_state_persists_across_restart_lfs` | `crates/rockstream-gateway/tests/core_operator_durability_minio_tests.rs::compiled_join_state_persists_across_restart_minio` | — | — | — |
| `language.joins.equi-integer` | `state_growth` | Join arrangements use the configured state budget and expose fill level. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_join_equi_integer_state_growth` | — | configured arrangement state budget | arrangement_state_fill_ratio | the write is rejected with its RS-coded resource error |
| `language.joins.equi-integer` | `failure` | Invalid join input returns an actionable RS-coded error. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_join_equi_integer_failure` | — | — | — | — |
...
| `language.window.slide` | `incremental` | Sliding window deltas recompute overlapping slice bounds and preserve exact ordered outputs. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_window_slide_incremental` | — | — | — | — |
| `language.window.slide` | `backfill` | Sliding window backfill evaluates all historical slice intervals across the snapshot. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_window_slide_backfill` | — | — | — | — |
| `language.window.slide` | `checkpoint_recovery` | A reopened sliding window shard restores committed slice state. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_window_slide_checkpoint_recovery` | — | — | — | — |
| `language.window.slide` | `state_growth` | Sliding window buffers retain only unexpired active slices and report fill level. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_window_slide_state_growth` | — | configured slide window slice budget | window_state_fill_ratio | the write is rejected with its RS-coded resource error |
| `language.window.slide` | `failure` | Unsupported sliding window specifications fail closed with an actionable RS-coded error. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_window_slide_failure` | — | — | — | — |
| `language.window.tumble` | `incremental` | Tumbling window deltas assign rows into non-overlapping epoch buckets deterministically. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_window_tumble_incremental` | — | — | — | — |
| `language.window.tumble` | `backfill` | Tumble window backfill processes the complete snapshot before subsequent stream deltas. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_window_tumble_backfill` | — | — | — | — |
| `language.window.tumble` | `checkpoint_recovery` | A reopened tumble window shard restores the committed window transcript in both local and object-store backends. | `crates/rockstream-gateway/tests/live_exec_tumble_window_durability_lfs_tests.rs::compiled_tumble_window_state_persists_across_restart_lfs` | `crates/rockstream-gateway/tests/core_operator_durability_minio_tests.rs::compiled_tumble_window_state_persists_across_restart_minio` | — | — | — |
| `language.window.tumble` | `state_growth` | Tumble window state is bounded by the active watermark retention window and reports fill level. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_window_tumble_state_growth` | — | configured tumble window partition budget | window_state_fill_ratio | the write is rejected with its RS-coded resource error |
| `language.window.tumble` | `failure` | Invalid window parameters fail closed with an actionable RS-coded error. | `crates/rockstream-gateway/tests/core_operator_semantics_e2e_tests.rs::core_window_tumble_failure` | — | — | — | — |
| `sink.kafka` | `incremental` | Materialized view change records are delivered to the Kafka sink topic matching the incremental output. | `crates/rockstream-connectors/tests/kafka_sink_guarantee_matrix_tests.rs::kafka_sink_incremental_payload_matches_view` | — | — | — | — |
| `sink.kafka` | `backfill` | Initial snapshot emission to Kafka sink delivers exactly all existing view records before live deltas. | `crates/rockstream-connectors/tests/kafka_sink_guarantee_matrix_tests.rs::kafka_sink_initial_snapshot_delivery_exact` | — | — | — | — |
| `sink.kafka` | `checkpoint_recovery` | A recovery checkpoint persisted to and restored from an object-store backend commits byte-identical output across a simulated restart, in both local and object-store backends. | `crates/rockstream-connectors/tests/connector_scenario_durability_lfs_tests.rs::kafka_sink_checkpoint_restart_lfs_has_byte_identical_commit` | `crates/rockstream-connectors/tests/connector_scenario_durability_minio_tests.rs::kafka_sink_checkpoint_restart_minio_has_byte_identical_commit` | — | — | — |
| `sink.kafka` | `state_growth` | Kafka sink staging buffer is bounded and exposes its fill metric. | `crates/rockstream-connectors/tests/dlq_backpressure_threshold_tests.rs::kafka_sink_buffer_bounded_with_fill_metric` | — | configured staging buffer limit | kafka_sink_buffer_fill_ratio | pre-commit applies backpressure or returns its RS-coded resource error |
| `sink.kafka` | `failure` | A producer communication or broker failure fails closed with an actionable RS-coded error. | `crates/rockstream-connectors/tests/kafka_sink_guarantee_matrix_tests.rs::kafka_sink_producer_error_fails_closed` | — | — | — | — |
