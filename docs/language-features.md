...
- **Tier: Core** — Aggregation and algebraic maintenance: `COUNT`, `COUNT(*)`, `SUM`, `AVG`/`MEAN`, `MIN`, `MAX`, algebraic merge-law propagation, and CRDT-backed aggregate state (`WeightAdd/v1`, `SumCount/v1`, `MaxRegister/v1`, `MinRegister/v1`). Proof: [`test_basic_int_sum_group_by`](../crates/rockstream-gateway/tests/serving_path_aggregate_matrix_tests.rs).
- **Tier: Core** — Relational operators: inner, left, right, full outer, semi, anti, and cross joins; `UNION`, `INTERSECT`, `EXCEPT`, `DISTINCT`, and bag/set semantics. Proof: [`tpch_q3_two_join_aggregate`](../crates/rockstream-sql/tests/tpch_plans.rs).
- **Tier: Core** — Analytics and time: `ROW_NUMBER`, `RANK`, `DENSE_RANK`, `LAG`, `LEAD`, sliding `SUM`/`AVG`, `TUMBLE`, watermarks, event-time TTL, late-data policies (`drop`, `update`, `route_to_sink`), and watermark gating. `NTILE` is *not* supported — `crates/rockstream-sql/src/lower.rs` explicitly returns `RS-1016` (`UnsupportedWindowFunction`) for it; a prior revision of this doc incorrectly listed it as implemented. Proof: [`sql_engine_window_row_number_over_view`](../crates/rockstream-sql/tests/sql_engine_e2e.rs).
