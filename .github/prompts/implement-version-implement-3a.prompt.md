...
Once the first half of slices are implemented and tests pass:

1. Output **exactly** this message and nothing else:
   > "Phase 3a done. First half of slices implemented and tests passing. Run `/compact` now, then run `/implement-version-implement-3b` with version `${input:version}`."
