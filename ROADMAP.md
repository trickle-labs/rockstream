...
- **Developer Experience (DX) baseline**: All subcommand paths must support shell tab-completion, and error diagnostics must remain completely self-explanatory. Interactive terminal REPLs (such as the `rockstream sql` shell) must integrate command history, syntax coloring, and contextual tab-completion. Logging and tracing spans must utilize correlation IDs and filter noise out of the default local setup output.
- **End-user Documentation standard**: Any added CLI options, SQL dialect syntax, system-catalog virtual tables, or configuration knobs are legally required to carry user-facing reference documentation in a `.md` format inside the `docs/` directory at check-in. In-code user-facing errors must include inline, actionable `next_steps` remediation text that mirrors the docs.
- **MinIO & Object-Storage Integration Testing**: Any functional pathway touching WALs, custom compaction, checkpoints, alignment buffers, backing store list/gets, DLQs, or S3-format source-sink connectors must execute a dedicated suite of integration tests running against a real containerized Minio-based object-store container. These integration runs must verify robustness (gRPC/HTTP timeouts, retries, and data checksum integrity) under S3 API path faults.
...
### Historical Appendix: Superseded Post-v0.52 Detail

| Version | Status | Focus | Scope | Proof |
|---|---|---|---|---|
| v0.48.1 | ✅ Done | Integration Beta — Wire What We Shipped | **Phase A — Correctness debts** (prerequisite for everything): fix `decode_object` 5× `.try_into().unwrap()` panics in `exchange/durable.rs` — replace with `DurableError::MalformedObject` propagation and add fuzz corpus (10k seeds, every truncation length); hard-fail merge-read fallback with `RS-5003 law.operand_corruption` instead of silently returning raw bytes — add `--allow-law-operand-fallback` override flag for emergency recovery; wire `Arc<StateBudgetMeter>` into every stateful operator (`AggregateMergeOp`, `MinMaxOp`, join, top-k, distinct) — emit `RS-3604` / `OVER_BUDGET_RELAXED` on exhaustion; cap `committed_epochs` / `aborted_epochs` in all sinks to a ring buffer of 1,024 entries with fill-level metric; fix `generate_rows.rs` panic on non-8-byte keys; replace `panic!` in `dml.rs` test match arm with `assert!`. **Phase B — Integration wiring**: mark all stub `rockstream_catalog.*` functions (except `merge_laws`) to return `RS-9001 catalog.data_not_wired` diagnostic entries until backed by live state; wire `catalog_epochs` and `catalog_shards` to control-plane; implement persistent per-source DLQ storage (`0x07 DLQ source_id(16) seq(8 BE) → record_bytes`) and wire every connector source decode-error path through it; implement `REPLAY` / `DISMISS` against real DLQ storage (`catalog_dead_letter_queue` scans this storage prefix); implement per-shard idempotency-key table (`0x02 0xIK shard(4) idem_key_hash(16) → epoch(8)`) with atomic `WriteBatch` insert and gateway `DbReader` lookup before assuming key uniqueness (24h TTL via compaction filter); implement `EXPLAIN TRANSACTION` SQL frontend — parser recognizes statement, planner collects `LawSchemaMetadata` from every plan source/sink, `ExplainTransaction::from_schema_metadata()` renders `WriteClassification` fields; wire `Sink::should_flush(bytes_buffered, epochs_buffered)` into the epoch driver with tracked buffers and hard ceiling for backpressure; add planner partition-filter derivation from WHERE predicates dispatching to `start_snapshot(Some(filter))` / `poll_delta(Some(filter))` when `partition_filter_support()` is true; implement `ALTER SOURCE … PAUSE` / `RESUME` / `DROP SOURCE` DDL with control-plane state-machine, audit emission, and all real connectors (Kafka, Postgres CDC, S3, HTTP, Iceberg) honoring lifecycle — pipeline driver checks `lifecycle_state()` before `poll_batch`. **Source/Sink ergonomics**: split into Tier 1 base trait + opt-in capability supertrait extensions (`PartitionPushdownSource`, `SchemaAwareSource`, `LifecycleSource`, `Tier2Sink`) so the runtime explicitly detects each capability rather than inheriting silent defaults. **DX, Docs & MinIO and S3-compatibility integration testing**: (1) DX: add structured diagnostic JSON logging options to the binary; verify tracing context propagation (`traceparent` headers) across gRPC exchanges; ensure inline error codes display immediate remediation guidance. (2) Docs: publish the formal **"Connector Capability Status Matrix"** in `docs/connector-developer-guide.md` specifying operational paths versus known stubs. (3) MinIO integration: stand up a containerized MinIO test structure. Implement an integration test suite verifying that WAL lists, checkpoint manifests, and S3-format connector reads/writes run successfully over a real MinIO instance under induced HTTP 503/timeout faults. (4) Phase E: write a Post-Audit reconciliation notice inside `sign-offs/v0.41.md`, `v0.44.md`, `v0.47.md`, and `v0.48.md` pointing to this wire version. | Fuzz corpus: zero panics across all truncations of a durable shuffle object; merge-read fallback with corrupt operand returns `RS-5003` and pipeline enters `DEGRADED` — gateway query returns typed error, not stale data; StateBudget property test: operator fed 10× over-budget never exceeds budget + one delta, emits exactly one `RS-3604`; DLQ end-to-end: Kafka source decode failure appears in `rockstream_catalog.dead_letter_queue`, `REPLAY` re-queues it, `DISMISS` removes it; idempotency restart test: 1M counter increments across two gateway instances, first killed mid-run and restarted — final total is exact; Iceberg runtime test: source → IcebergSink at 10ms epoch for 60s asserts ≤ 2 files/minute (≥ 256 MB each), not one file per epoch; partition filter test: `WHERE date_partition = '2026-01-01'` against an S3 source asserts `start_snapshot(Some(filter))` is called and Z-set output is identical when fallback path is forced; lifecycle test: `ALTER SOURCE kafka_orders PAUSE` → zero `poll_batch` calls for 5s → `RESUME` → data flows within 1 epoch; MinIO Integration Test: Storage, DLQ, S3-connector, and checkpoint test suites pass end-to-end against a live MinIO service under a simulated 10% S3 endpoint failure-injection rate; `cargo test --workspace` green; `cargo clippy -D warnings` green. |
| v0.49 | ✅ Done | Auth, RBAC, secrets, transport security, and engine ergonomics foundation | OIDC/bearer auth, service accounts, per-view RBAC, mTLS everywhere, certificate rotation docs, secrets management (DESIGN.md §14.18): `CREATE SECRET` DDL, envelope encryption, KEK source config, worker-side token resolution, rotation without restart; **engine ergonomics foundation** (prerequisite for auth middleware and row-level security): refactor `rockstream-sql/src/lib.rs` into submodules (`parser.rs`, `binder.rs`, `lowering/mod.rs`, `ddl.rs`, `explain.rs`); `ClusterConfig` / `WorkerConfig` / `ConnectorConfig` skeleton in `rockstream-types` with `rockstream.toml` loader — all existing scattered knobs (`min_epoch_ms`, `segment_cache_bytes`, `checkpoint_retention_count`, `max_rows_per_quantum`, `state_budget_gb`, etc.) migrate to typed fields; CLI subcommand structure expanded with `rockstream describe <pipeline>`, `rockstream debug arrangement`, `rockstream support-bundle` stubs; **external gRPC connector protocol**: `proto/connector.proto` + `tonic` server stub; SDK and connector guide updated to reference the proto; reference gRPC connector example in `rockstream-connectors`; **`discover_schema()` planner integration**: `CREATE SOURCE/SINK` calls `discover_schema()`, validates declared vs. connector schema, raises `RS-XXXX schema.connector_mismatch` on divergence, caches result in catalog; **`WriteClassification` gateway integration**: `OptimisticTransaction::validate` branches on `read_dependent_delta` (force read-validation round) vs. `blind_delta` (skip read check); `rockstream.*` alias CI assertion: aliased queries emit deprecation `NOTICE` — CI assertion must pass before alias removal in v0.50. **DX, Docs & MinIO integration**: (1) DX: checking-in autogenerated shell autocompletion (zsh & bash) scripts for standard subcommands; interactive `rockstream sql` terminal REPL enhancements including statement history, live keywords syntax highlighting (using ANSI color terminals), and intelligent tab-completion; noise-free local logging defaults (`RUST_LOG=rockstream=info`). (2) Docs: publish **"RockStream SQL Language Reference"** (detailing Z-set algebraic logic, built-in functions, CRDT column expressions, and custom windows) and **"Cluster Security Hardening Guide"** (covering mTLS configs, secret management workflows, and envelope encryption setups). (3) MinIO integration: TLS/mTLS test runs with multi-tenant workspace credentials, using MinIO directly as a secure secrets distribution backend and verifying certificate rotation tasks against MinIO storage under active client workloads. | Auth integration tests reject unauthenticated and cross-tenant access; audit log has actor on every event; secrets are encrypted at rest; `SHOW SECRETS` never exposes values; rotation test updates a Kafka credential and active connector re-acquire without pipeline restart; `rockstream-sql` compiles into submodules with no public API change — all existing SQL tests pass unchanged; `rockstream.toml` round-trip test: load → write → reload preserves all fields with correct types and defaults; gRPC connector example compiles and passes Tier 1 contract tests identical to the in-process example; `CREATE SOURCE s FROM KAFKA … (amount COUNTER)` with mismatched column type raises `RS-XXXX schema.connector_mismatch`; `INSERT` with `read_dependent_delta` column triggers a read round-trip before commit; `blind_delta` column skips it; `rockstream.*` alias emits deprecation `NOTICE` — verified by CI assertion; zsh/bash autocompletion and `rockstream sql` shell's colored multiline history are verified to run without panic; the complete SQL Language Reference and Security Guide are formally reviewed and merged into the `docs/` repository. |
| v0.50 | ✅ Done | Observability, admin surface, law diagnostics, SLO enforcement, and IVM engine visibility | Prometheus metrics, OTEL traces, JSON logs, admin CLI, support bundle completeness, dashboard template, `rockstream debug arrangement` IVM debugger (DESIGN.md §14.7.1) decodes arrangement law headers, tombstone density metric and proactive compaction trigger (DESIGN.md §5.4); full `merge_law_*` metric family live (applied, fallback, compaction-bytes-reclaimed, duplicate-dropped, tombstone-bytes, monotone-partial-lag); **actionable error messages** (DESIGN.md §14.14): every `RS-XXXX` error includes a `next_steps` field with remediation guidance, enforced in CI; **resource usage visibility** (DESIGN.md §14.19): `SHOW RESOURCE USAGE`, `SHOW RESOURCE USAGE FOR WORKLOAD <name>`, `SHOW CLUSTER RESOURCE USAGE`; `rockstream_catalog.view_resource_usage` and `workload_resource_usage` catalog tables; proactive NOTICE at 80% (`RS-5018`) and WARNING at 95% (`RS-5019`) of any budget; **schema evolution detection** (DESIGN.md §4.2): `SHOW SCHEMA_EVOLUTION STATUS FOR SCHEMA <name>` and `SHOW SCHEMA_EVOLUTION HISTORY FOR MATERIALIZED VIEW <name>`; `RS-6001 schema.incompatible_evolution` proactive notice; **workload SLO enforcement**: plumb `WorkloadId` through `OperatorContext` to every stateful operator; epoch coordinator sizes epochs against `freshness_slo_ms`; state budget enforcer charges per-operator allocations against workload `memory_limit` — these unblock the proactive `RS-5018`/`RS-5019` notices and the v0.51 auto-tuner; **`EXPLAIN INCREMENTAL ANALYZE` live round-trip**: add `ControlService::collect_operator_stats(pipeline_id) -> Vec<OperatorStats>` RPC; gateway `ANALYZE` handler calls it and renders live `rows_processed`, `state_read_count`, `rmw_ratio`, `p99_latency_ms` per operator node; **`LawMetricKey` per-operator dimension**: extend with `operator_id: Option<OperatorId>` for per-instance RMW attribution; render in `EXPLAIN INCREMENTAL ANALYZE`; **worker-side arrangement segment cache**: move `segment_cache.rs` logic into `DbReader` access path in `rockstream-runtime`; LRU keyed by `(shard_id, segment_id)` with TTL tied to checkpoint epoch; criterion benchmark asserting > 80% hit rate on TPC-H Q5 cross-shard join workload; **`Operator` introspection hooks**: `fn snapshot_metrics(&self) -> OperatorMetrics`, `fn reconfigure(&mut self, hints: OperatorHints) -> ReconfigOutcome`, `fn state_bytes(&self) -> u64` on all stateful operators — foundation for v0.51 auto-tuner; **`docs/configuration.md`** and **`docs/cli.md`** published; `rockstream.*` alias removed (announced v0.41, deprecated v0.49). **DX, Docs & MinIO integration**: (1) DX: bundle Grafana monitoring dashboard templates (out-of-the-box system metrics dashboard for monitoring ingestion rates, SlateDB compaction lag, memory/state budget utilization, and gRPC exchange queue depths). (2) Docs: publish **"Production SRE Operations Guide"** in `docs/sre-operations.md` covering Prometheus metrics mappings, OTEL tracing spans, alert notification rules, log filtering guidelines, and support bundle parsing procedures. (3) MinIO integration: dedicated integration test suite for the worker-side arrangement segment cache and group checkpoints on containerized MinIO — verify that segment cache hits over 80% on Join workloads backed by S3, and assert that alignment buffers and snapshot files are deleted correctly on MinIO upon retention epoch limits without memory or directory resource leaks; verify robust operational recovery and self-fencing under simulated network partitions to MinIO storage. | Operator can diagnose a slow pipeline from SLO compliance → degraded reason → explain → support bundle; can inspect a specific arrangement key including its merge law without stopping the pipeline; support bundle contains per-law statistics for the last 24h; every error in the registry has a non-empty `next_steps` (CI enforced); `SHOW RESOURCE USAGE` returns per-workload state/memory/SLO summary; a workload at 82% memory utilisation triggers `RS-5018` NOTICE; `SHOW SCHEMA_EVOLUTION STATUS` surfaces pending incompatible changes; workload `FRESHNESS_SLO = '100ms'` forces epoch flush within 100ms under SimRuntime timing assertions; `EXPLAIN INCREMENTAL ANALYZE orders_mv` returns non-zero `rows/s` and `p99_latency_ms` on a live running pipeline; RMW metric shows `rmw_avoided=true` per specific operator instance node (not law-wide); segment cache criterion benchmark reports > 80% hit rate on TPC-H Q5 cross-shard join workload; MinIO-backed arrangement segment cache and group checkpoints are validated under continuous write workloads with assertion of zero SlateDB format leaks on S3 storage. |
| v0.51 | ✅ Done | Auto-tuner hardening | Adaptive parallelism, epoch sizing, source throttle, hysteresis, stability tests, override docs. Requires `Operator::snapshot_metrics` / `reconfigure` hooks (v0.50) and workload SLO enforcement (v0.50) as prerequisites. **DX, Docs & MinIO integration**: (1) DX: CLI override mechanisms via `rockstream tune --override` to explicitly pin parallelisms, epoch sizes, or memory partitions for debugging auto-tuner behavior under local development workloads. (2) Docs: publish **"Auto-Tuning and Resource Sizing Policies"** in `docs/auto-tuning.md` detailing the tuning state machine, hysteresis dampening thresholds, the resource allocation priority algorithm, and safety override recovery steps. (3) MinIO integration: run MinIO-backed chaos tests asserting that under deep backpressure, source throttling commands successfully throttle upstream Kafka/S3 ingestion to match SlateDB storage write rates on MinIO without causing task-scheduling starvation. | Random workload property tests converge without oscillation; every tuning action is audit logged; `rockstream tune --override` accurately overrides the auto-tuner; MinIO-backed chaos-runs verify successful feedback loops under deep SlateDB write stall. |
| v0.52 | ✅ Done | Secondary indexes | `CREATE INDEX <name> ON <table> (<col>[, …])` DDL backed by the IVM engine: system-managed materialized view with `ARRANGE BY (index_cols, pk_cols)`, `BUILDING` → `READY` backfill lifecycle; `CREATE INDEX … WHERE <pred>` (partial indexes); `DROP INDEX` with frontier-aware arrangement GC; `REBUILD INDEX`; planner selects `index_scan` vs `shard_scan` using `index_prefer_selectivity_threshold` (default 0.01) and falls back when frontier lag exceeds `index_max_lag_ms`; index state bytes in `EXPLAIN INCREMENTAL ESTIMATE` and charged against `state_budget_gb`; error codes `RS-2014` (`index.building`), `RS-2015` (`index.frontier_lag`), `RS-2016` (`index.name_conflict`); simulation tests for backfill under concurrent writes, shard split during backfill, and crash/recovery. **DX, Docs & MinIO integration**: (1) DX: add `EXPLAIN INDEX <index_name>` command rendering index-level statistics, fragmentation ratios, index selectivity, and cache hit metrics. (2) Docs: publish **"Secondary Index Tuning & Optimizer Deep-Dive"** in `docs/index-tuning.md` detailing the optimizer's index selection decisions, selectivity calculation details, and cost metrics inside `EXPLAIN INCREMENTAL`. (3) MinIO integration: end-to-end integration tests using containerized MinIO verifying backfill consistency under concurrent DML and shard splits, assuring that historical arrangement segments are compacted correctly, and that old/dropped index files are completely deleted from MinIO. | `SELECT * FROM orders WHERE customer_id = 42` uses `index_scan` path when selectivity < threshold; planner falls back to `shard_scan` when index is in `BUILDING` state; partial index stores fewer rows and returns correct results; `DROP INDEX` GCs arrangement state within 2 epochs; index state bytes appear in `EXPLAIN INCREMENTAL ESTIMATE`; simulation: no data loss or duplicate rows after shard split during backfill; `EXPLAIN INDEX` shows correct selectivity; MinIO storage lists zero orphaned or leaked files/directories after index drops. |
| v0.52.1 |  | Shard column statistics, upgrades, migration, and security review | Storage format gate, rolling upgrade test, migration skeleton, disaster recovery drill, independent security review; **shard column statistics for OLAP scatter pruning** (DESIGN.md §8.7, §12.3.1): per-shard min/max bounds, blocked Bloom filters (64 KB budget per column per shard), and HLL cardinality sketches piggybacked on checkpoint `WorkerFrontierSummary`; gateway planner prunes scatter set using column stats before scatter; `shard_stats_max_age` freshness guard with `RS-2017` / `shard_stats.too_stale` fallback; secondary-index stat injection (indexed columns publish stats automatically at build completion); metrics `scatter_shards_total`, `scatter_shards_pruned_total`, `shard_bloom_false_positive_total`. **DX, Docs & MinIO integration**: (1) DX: tools for diagnostic inspects of local shard statistics via `rockstream inspect stats <file-path>`, detailing the blocked Bloom filter structure and hyperloglog buckets. (2) Docs: publish **"Disaster Recovery and Rolling Upgrade Manual"** in `docs/disaster-recovery.md` with concrete walk-through checklists for cluster state backup, migration scripts, fallback procedures, and emergency data recovery from raw SlateDB files on S3/MinIO. (3) MinIO integration: execute full disaster recovery simulations pulling state from a partitioned MinIO storage layout, restoring system metadata from S3 dumps, and verifying subsequent query convergence. | N → N+1 rolling upgrade loses no epoch; incompatible format fails safely with `RS-5001`; security review issues are triaged; a selective query on a 100-shard view with 8 matching shards uses ≤ 12 shards in `EXPLAIN`; `RS-2017` is emitted as `NOTICE` when stats are stale; after `CREATE INDEX` the next checkpoint publishes stats for the indexed column; property test verifies Bloom never produces false negatives over 10k randomized workloads; disaster recovery test completely restores working state from physical MinIO dumps under client load. |
| v0.52.2 |  | Long production soak, user-defined merge laws, and optimistic transaction subset | 30-day 64-shard soak, 1,000-shard control/exchange stress, Nexmark/TPC-H continuous, chaos automation, continuous simulation soak scaled to millions of seeds (infrastructure started at v0.36), release-blocking defect burn-down; **`CREATE MERGE LAW`** DDL behind a feature flag, gated on the shared property-test suite, explicit `duplicate_policy` / `compaction_policy`, registered `EXPLAIN` formatter, and one fault-model entry; **`--experimental-optimistic-crdt-transactions`** flag enabling: `SERIALIZABLE LOCAL` when planner proves one shard, optimistic exact-key guarded writes (`RS-2008` on conflict), CRDT-only transaction envelope prototype (if atomic visibility is implemented), and clear rejection (`RS-2009`) for unsupported shapes. **DX, Docs & MinIO integration**: (1) DX: developer toolkit for validating custom user-defined merge laws locally (`rockstream law validate --file <law.json>`) which runs the complete seed-based algebraic test suite inside local environments and outputs diagnostic compliance reports. (2) Docs: publish **"User-Defined Merge Laws & Advanced CRDT Extension Guide"** in `docs/user-defined-laws.md` explaining algebraic laws compliance, custom compaction hook implementations, and registration mechanisms. (3) MinIO integration: containerized 30-day (compressed timeline) end-to-end MinIO storage soak under continuous heavy write and compaction workloads, asserting zero file corruption, zero memory leaks, and perfect ledger reconciliation. | 99.99% availability target met or miss is understood and fixed; no correctness divergence; large-cluster stress stays within exchange and frontier budgets; all historical failing simulator seeds replay in CI; a sample user-defined law (e.g. min-clamped counter) passes the harness and survives 24h of fault injection; `SERIALIZABLE LOCAL` commits succeed for planner-proven single-shard transactions; optimistic exact-key conflict returns `RS-2008` on stale version; unsupported shapes return `RS-2009`; simulation corpus includes optimistic validation crash interleavings; 30-day compressed soak runs flawlessly against live MinIO without a single lease loss or SlateDB corruption event. |
| v0.52.3 |  | Production beta handoff | Helm/Terraform packaging, deployment playbooks, SQL reference, connector guide, operator guide, reference architecture. **DX, Docs & MinIO integration**: (1) Docs: formally combine individual guides into the comprehensive official **"RockStream Production Handbook"** in `docs/production-handbook.md` spanning deployment playbooks, monitoring dashboards structure, and disaster recovery runs. (2) MinIO integration: conduct local single-command deployment and rollback verification testing against MinIO-configured Helm/Terraform environments, confirming complete operational compliance. | First pilot workload runs with support agreement, documented runbook, rollback plan, and known limitations; Helm/Terraform test deployment against MinIO successfully boots, hosts views, survives replica teardowns, and cleans up completely on command. |

### Cold Tier & Data Lake Integration

| Version | Status | Focus | Scope | Proof |
|---|---|---|---|---|
| v0.52.4 |  | Cold-tier Parquet/Iceberg sink with law metadata | Iceberg v2 cold-tier sink writer (DESIGN.md §13.6): `CREATE SINK ... TO ICEBERG` with `snapshot_interval_epochs`/`snapshot_interval_ms`, Parquet data files with column stats, manifest files and manifest lists, atomic `metadata.json` commit, `should_flush`-gated buffering with pending rows staged in shard SlateDB, exactly-once via idempotent file keys; cold-tier snapshots embed `(law_id, law_version)` per CRDT column and write *finalized* values (folded counters, register winners, set memberships) so external readers never see raw operands; `ViewReader` `TwoTier` variant functional: gateway can merge cold snapshot + hot LSM tail; cold snapshot GC (§13.6.2.1). **DX, Docs & MinIO integration**: (1) DX: add CLI validation tool `rockstream inspect cold --sink <sink-name>` to print active cold-tier manifest listings, pending snapshot buffering levels, and metadata tags directly. (2) Docs: publish **"Two-Tier Cold Tier Architecture & Integration Guide"** in `docs/two-tier-architecture.md` describing Parquet schemas, Iceberg serialization details, epoch compaction, and CRDT column folding layouts. (3) MinIO integration: execute full integration tests against containerized MinIO verifying Parquet writes, Iceberg metadata commits, and catalog updates under continuous network hiccups and simulated connection drops to MinIO. | Cold-tier sink writes valid Iceberg v2 table readable by DuckDB `iceberg_scan`; full-scan query over a 100M-row view uses cold tier and completes 10x faster than hot-only LSM scan; snapshot GC keeps ≤ `cold_snapshot_retention_count` snapshots per view; crash mid-flush produces no orphan data files; a CRDT column read via DuckDB shows the finalized value; MinIO-backed Parquet and Iceberg table commits survive 20% simulated network drop rates without metadata divergence or duplicated file commits. |
| v0.52.5 |  | Catalog registration and Iceberg REST catalog server | Catalog registration backends (§13.6.5): `filesystem` (self-contained, already functional), `glue`, `rest`, `hive`, `ducklake`; native Iceberg REST catalog server (§13.7) on gateway HTTP port 8181: `/iceberg/v1/` serves namespaces, tables, snapshots backed by control-plane metadata; auth token/mTLS passed through. **DX, Docs & MinIO integration**: (1) DX: include `/metrics` endpoint metrics showing active HTTP traffic, authorization successes/failures, table listing response times, and catalog latency metrics. (2) Docs: publish **"Iceberg REST Catalog Server Reference Guide"** in `docs/iceberg-rest-catalog.md` detailing standard routing endpoints, credentials resolution, schema mappings, and multi-tenant authorization hooks. (3) MinIO integration: run integration testing connecting Spark, Trino, and DuckDB to a local Rocksdb/gRPC cluster with containerized MinIO acting as the physical Iceberg catalog backend storage, verifying queries over Iceberg tables with active write workloads. | Spark/Trino/DuckDB discover views by name via `catalog.uri=http://rockstream:8181/iceberg/v1`; Glue catalog shows table within 30s of snapshot commit; `CATALOG_WARN` state surfaces cleanly when external catalog is unreachable; catalog API failures never block IVM; Spark-to-MinIO and Trino-to-MinIO queries over RockStream REST catalog server achieve bit-identical results compared to local Postgres read queries under live 10k ops/sec writes. |
| v0.52.1 |  | Shard column statistics, upgrades, migration, and security review | Storage format gate, rolling upgrade test, migration skeleton, disaster recovery drill, independent security review; **shard column statistics for OLAP scatter pruning** (DESIGN.md §8.7, §12.3.1): per-shard min/max bounds, blocked Bloom filters (64 KB budget per column per shard), and HLL cardinality sketches piggybacked on checkpoint `WorkerFrontierSummary`; gateway planner prunes scatter set using column stats before scatter; `shard_stats_max_age` freshness guard with `RS-2017` / `shard_stats.too_stale` fallback; secondary-index stat injection (indexed columns publish stats automatically at build completion); metrics `scatter_shards_total`, `scatter_shards_pruned_total`, `shard_bloom_false_positive_total`. **DX, Docs & MinIO integration**: (1) DX: tools for diagnostic inspects of local shard statistics via `rockstream inspect stats <file-path>`, detailing the blocked Bloom filter structure and hyperloglog buckets. (2) Docs: publish **"Disaster Recovery and Rolling Upgrade Manual"** in `docs/disaster-recovery.md` with concrete walk-through checklists for cluster state backup, migration scripts, fallback procedures, and emergency data recovery from raw SlateDB files on S3/MinIO. (3) MinIO integration: execute full disaster recovery simulations pulling state from a partitioned MinIO storage layout, restoring system metadata from S3 dumps, and verifying subsequent query convergence. | N → N+1 rolling upgrade loses no epoch; incompatible format fails safely with `RS-5001`; security review issues are triaged; a selective query on a 100-shard view with 8 matching shards uses ≤ 12 shards in `EXPLAIN`; `RS-2017` is emitted as `NOTICE` when stats are stale; after `CREATE INDEX` the next checkpoint publishes stats for the indexed column; property test verifies Bloom never produces false negatives over 10k randomized workloads; disaster recovery test completely restores working state from physical MinIO dumps under client load. |
| v0.52.2 |  | Long production soak, user-defined merge laws, and optimistic transaction subset | 30-day 64-shard soak, 1,000-shard control/exchange stress, Nexmark/TPC-H continuous, chaos automation, continuous simulation soak scaled to millions of seeds (infrastructure started at v0.36), release-blocking defect burn-down; **`CREATE MERGE LAW`** DDL behind a feature flag, gated on the shared property-test suite, explicit `duplicate_policy` / `compaction_policy`, registered `EXPLAIN` formatter, and one fault-model entry; **`--experimental-optimistic-crdt-transactions`** flag enabling: `SERIALIZABLE LOCAL` when planner proves one shard, optimistic exact-key guarded writes (`RS-2008` on conflict), CRDT-only transaction envelope prototype (if atomic visibility is implemented), and clear rejection (`RS-2009`) for unsupported shapes. **DX, Docs & MinIO integration**: (1) DX: developer toolkit for validating custom user-defined merge laws locally (`rockstream law validate --file <law.json>`) which runs the complete seed-based algebraic test suite inside local environments and outputs diagnostic compliance reports. (2) Docs: publish **"User-Defined Merge Laws & Advanced CRDT Extension Guide"** in `docs/user-defined-laws.md` explaining algebraic laws compliance, custom compaction hook implementations, and registration mechanisms. (3) MinIO integration: containerized 30-day (compressed timeline) end-to-end MinIO storage soak under continuous heavy write and compaction workloads, asserting zero file corruption, zero memory leaks, and perfect ledger reconciliation. | 99.99% availability target met or miss is understood and fixed; no correctness divergence; large-cluster stress stays within exchange and frontier budgets; all historical failing simulator seeds replay in CI; a sample user-defined law (e.g. min-clamped counter) passes the harness and survives 24h of fault injection; `SERIALIZABLE LOCAL` commits succeed for planner-proven single-shard transactions; optimistic exact-key conflict returns `RS-2008` on stale version; unsupported shapes return `RS-2009`; simulation corpus includes optimistic validation crash interleavings; 30-day compressed soak runs flawlessly against live MinIO without a single lease loss or SlateDB corruption event. |
| v0.52.3 |  | Production beta handoff | Helm/Terraform packaging, deployment playbooks, SQL reference, connector guide, operator guide, reference architecture. **DX, Docs & MinIO integration**: (1) Docs: formally combine individual guides into the comprehensive official **"RockStream Production Handbook"** in `docs/production-handbook.md` spanning deployment playbooks, monitoring dashboards structure, and disaster recovery runs. (2) MinIO integration: conduct local single-command deployment and rollback verification testing against MinIO-configured Helm/Terraform environments, confirming complete operational compliance. | First pilot workload runs with support agreement, documented runbook, rollback plan, and known limitations; Helm/Terraform test deployment against MinIO successfully boots, hosts views, survives replica teardowns, and cleans up completely on command. |
...
Post-0.48, every version includes dedicated user-facing documentation milestones to ensure absolute beta-readiness and operational clarity. The documentation deliverable for each milestone is:
...
| `v0.48` | Connector developer guide: Tier 1 and Tier 2 contracts, SDK usage, DLQ configuration, and end-to-end example connector in [docs/connector-developer-guide.md](docs/connector-developer-guide.md). |
| v0.48.1 | Connector Capability Status Matrix specifying operational pathways versus known stubs in [docs/connector-developer-guide.md](docs/connector-developer-guide.md). |
| v0.49 | SQL & Security: "RockStream SQL Language Reference Guide" (detailing algebraic logic, scalar/aggregate merge laws, and window/lateral joins) and "Cluster Security Hardening Guide" (covering mTLS configuration, secret envelope encryption, and key rotation). |
| v0.50 | Observability & Operations: "Production SRE Operations Guide" (compiling all system metrics mappings, OTEL spans, log filtering rules, and support bundle parsing procedures). |
| v0.51 | Resource Policies: "Auto-Tuning and Resource Sizing Policies Guide" (detailing the tuning state machine, hysteresis dampening thresholds, and resource allocation priority algorithm). |
| v0.52 | Optimizations & Indices: "Secondary Index Tuning & Optimizer Deep-Dive Guide" (detailing secondary index selection decisions, selectivity thresholds, and index cost metrics). |
| v0.52.1 | Upgrade & Recoverability: "Disaster Recovery and Rolling Upgrade Manual" (with concrete walk-through checklists for cluster state backup, migration scripts, and emergency data manual-recovery). |
| v0.52.2 | Advanced CRDT Customization: "User-Defined Merge Laws & Advanced CRDT Extension Guide" (detailing algebraic laws compliance, custom compaction hook implementations, and registration mechanisms). |
| v0.52.3 | Comprehensive Admin Handoff: "RockStream Production Handbook" formally combining deployment playbooks, monitoring dashboards structure, and disaster recovery runs. |
| v0.52.4 | Storage Tiering Architecture: "Two-Tier Cold Tier Architecture & Integration Guide" (describing Parquet schemas, Iceberg serialization details, epoch compaction, and CRDT column folding layouts). |
| v0.52.5 | Catalog Interface Reference: "Iceberg REST Catalog Server Reference Guide" (detailing standard routing endpoints, credentials resolution, schema mappings, and multi-tenant authorization hooks). |
...
| User-defined laws readiness | `v0.48.1` | Is the built-in CRDT catalog (`COUNTER`, `LWW`, `G_SET`, `OR_SET`, `MAX/MIN_REGISTER`, `APPROX_*`) stable enough — under property tests, chaos, connector schema, and cold-tier, **and with connector lifecycle/DLQ/idempotency operationally wired** — to open `CREATE MERGE LAW` to users in `v0.54`? | |
| Product wedge | `v0.48.1` | Is the `psql` + live views + connectors experience compelling enough for pilot users, now that connector lifecycle DDL, persistent DLQ, per-shard idempotency, and real catalog data are wired end-to-end? | |
