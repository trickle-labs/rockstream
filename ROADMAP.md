...
| Version | Status | Focus | Scope | Proof |
|---|---|---|---|---|
| v0.52.1 | ✅ Done | Binary contract, bootstrap, and restart | Live `rockstream` CLI surface through TestContainers: `start`, `bootstrap`, `explain`, `sql`, `version`, `describe`, `debug arrangement`, `support-bundle`, and `tune`; role matrix; audit log and support-bundle artifacts. | Every public command is exercised end-to-end against a real binary container and produces the expected exit code, output, and artifacts. |
...
| v0.52.8 | ✅ Done | Comprehensive language features validation | Comprehensive E2E coverage for all SQL and query capabilities: SELECT with filters, joins, aggregations, window functions, time windows (TUMBLE), set operations, monotone recursion, historical reads (AS OF), subscribe streams, freshness control variables, optimistic transaction conflict (RS-2008), write classification (EXPLAIN TRANSACTION), source/sink lifecycle DDL, dead-letter queue SQL catalog queries, CRDT table column types, index lifecycle, and background DDL coordination. | E2E test suite executes a comprehensive multi-feature test verifying all documented language features end-to-end against a live gateway cluster, asserting precise returned rows, metadata, and error codes. |
| v0.52.9 | ✅ Done | Precise query execution and advanced language features verification | E2E test suite expanded to cover all language features in docs/language-features.md with precise value assertions. | All queries compile and lower, and the E2E test asserts exact schemas, types, and values, exercising core RockStream components under complex scenarios (joins, set operations, window functions, custom session variables). |
| v0.52.10 | ✅ Done | Campaign attribution DAG and comprehensive language features validation | E2E coverage for a real-world campaign attribution and recursive referral DAG, validation of remaining language features (TRY_CAST, NOW, READ COMMITTED, DDL/workload operations), OID mapping, and fault recovery checks. | E2E tests verify data flows correctly through a multi-stage view DAG with precise assertions, compile/lower and run all query statements, and verify fault resilience (worker/storage recovery). |
...
| v0.55 |  | Production beta handoff | Helm/Terraform packaging, deployment playbooks, SQL reference, connector guide, operator guide, reference architecture. **DX, Docs & MinIO integration**: (1) Docs: formally combine individual guides into the comprehensive official **"RockStream Production Handbook"** in `docs/production-handbook.md` spanning deployment playbooks, monitoring dashboards structure, and disaster recovery runs. (2) MinIO integration: conduct local single-command deployment and rollback verification testing against MinIO-configured Helm/Terraform environments, confirming complete operational compliance. | First pilot workload runs with support agreement, documented runbook, rollback plan, and known limitations; Helm/Terraform test deployment against MinIO successfully boots, hosts views, survives replica teardowns, and cleans up completely on command. |
...
| v0.53 | Upgrade & Recoverability: "Disaster Recovery and Rolling Upgrade Manual" (with concrete walk-through checklists for cluster state backup, migration scripts, and emergency data manual-recovery). |
| v0.54 | Advanced CRDT Customization: "User-Defined Merge Laws & Advanced CRDT Extension Guide" (detailing algebraic laws compliance, custom compaction hook implementations, and registration mechanisms). |
| v0.55 | Comprehensive Admin Handoff: "RockStream Production Handbook" formally combining deployment playbooks, monitoring dashboards structure, and disaster recovery runs. |
