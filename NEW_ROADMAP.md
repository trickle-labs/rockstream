...
| Standard-Client & Enterprise-Honest | v0.51.12 | A current `psql`/libpq (protocol 3.2) connects; ordinary `int`/`text`/`float` `GROUP BY`/`SUM` materialized views compile and maintain incrementally; a `SELECT` from an absent relation errors instead of returning empty `OK`; `--auth` is actually enforced (fail-closed); at least one real native Kafka/S3 source ingests exactly-once through `CREATE SOURCE`; operators can debug a stalled or wrong view over the wire; multi-tenant quotas and control-plane lease HA are enforced and proven against real processes; and operational edge cases carry machine-checkable specs validated by a real multi-worker chaos/SLO soak. |
...
| Phase 15.6 — Standard-Client Reachability, Serving-Path Completeness & Honest Enterprise Enforcement | v0.51.7 – v0.51.12 | New operational edge-case specs at v0.51.12 (quota exhaustion, source failure, object-store brownout, misconfiguration, late data); M7 lease-HA re-proven against real processes at v0.51.10 |
