...
¹ **Re-opened by the <=v0.42 implementation review (2026-07-10).** "All four FizzBee models green" was never actually true: the `formal-verify` CI job has never successfully installed the `fizzbee` binary (wrong release-asset filename), never runs on pull requests (its `if:` guard tests a non-existent field), and carries `continue-on-error: true`, so nothing has ever blocked a merge on a red or crashing model. Running the models directly for the first time found M1 failing its `M1_S5_IdempotentReplay` invariant and M3/M4 crashing outright (undefined-variable Starlark errors) — only M2 has ever genuinely passed. The distributed-engine Rust implementation and its `SimRuntime`/chaos test suite are unaffected by this finding; only the *formal-verification* proof of v0.18–v0.22 is unverified. See `formal/findings.md` ("Post-v0.42 Review") and roadmap version **v0.42.1** below.

² **Coverage gate discrepancy found by the <=v0.42 review.** The v0.42 sign-off states the gateway coverage gate is "≥90% line / ≥85% branch", but `.github/workflows/ci.yml` actually enforces `--fail-under-lines 70` / `--fail-under-regions 70` (region, not branch, coverage — `cargo-llvm-cov` has no `--fail-under-branches` flag), and the test that was supposed to lock this in (`conformance_doc_tests::test_coverage_gate_config_is_present`) asserts the *70%* strings. A real multi-row `INSERT ... VALUES (...), (...)` parsing bug in the gateway (silently corrupts/drops the last column, no error) was also found, undocumented and untested. See roadmap version **v0.42.2** below.
...
| Phase 11.5 — Post-v0.42 Remediation | v0.42.1 – v0.42.2 | Toolchain remediation (v0.42.1) |
