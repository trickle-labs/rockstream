...
| Pre-v1 Product Experience & Quality | v0.59.8–v0.59.17 | Single-source product surface manifest, simple executable documentation, golden-path project templates (local/Kafka/CDC Compose), structured diagnostics across all surfaces, public-path scenario/differential framework, full lifecycle/client/backend test closure, SQL semantics and limits, production lifecycle and health contracts, supported deployment profiles, and capacity/support guidance. |
...
| v0.59.13 | Lifecycle, Client, Documentation & Backend Test Closure | Add lifecycle race/restart/recovery matrix (`TST-007`–`TST-010`), external client workflows, real Kafka/PostgreSQL/MinIO paths, route docs/golden paths through scenarios, and enforce zero hidden skips. | Core behavior ledger is complete across required levels/backends; all golden paths and current docs run; all public surfaces agree; no required high-level test is skipped or retried. | Unit, LFS, MinIO, Postgres, Kafka, TC |
| v0.59.14 | SQL Semantics, Compatibility & System Limits | Publish the exact behavioral contract for NULLs, overflow, decimal precision and scale, timestamp precision/time zones/DST, identifier folding, collation and ordering, floating point, duplicate rows, unmatched DML, transaction visibility, and PostgreSQL compatibility boundaries; generate `docs/sql-semantics.md` and `docs/limits.md` from authoritative behavior and limit metadata where practical. | Every admitted semantic has executable conformance coverage against the batch oracle or PostgreSQL reference; every published limit is sourced from a runtime/configuration constant or an explicitly measured bound; generated documents are drift-checked, and exceeding each bound fails with its documented `RS-XXXX` code rather than truncating or silently degrading. | Unit, LFS, TC |
