...
| Pre-v1 Product Experience & Quality | v0.59.8–v0.59.13 | Single-source product surface manifest, simple executable documentation, golden-path project templates (local/Kafka/CDC Compose), structured diagnostics across all surfaces, public-path scenario/differential framework, and full lifecycle/client/backend test closure. |
| v1.0 Release | after v0.59.13 | The exact signed candidate artifacts pass every bounded automated qualification gate with zero open P0/P1 defects; optional extended soaks may supplement, but do not replace or block, the automated evidence; `v1.0.0` is then tagged. |
...
After v0.59.13, create `v1.0.0-rc.1` from one protected source SHA and freeze its
...
| v0.23–v0.59.13 | Continuous `formal-verify` + path-coupling (DC.1–DC.2); pre-release relaxed-bounds sweep (DC.4) | all `.fizz` specs | all M1–M7 | A coordination-protocol change without a model touch fails CI; the automated qualification suite re-runs the relaxed-bounds sweep against the candidate SHA |
...
| Phase 17 — Production Readiness & 1.0 Finalization | v0.53 – v0.59.13 (including the v0.53.1–v0.59.13 sub-versions added by the 2026-08-12 and 2026-08-18 reviews) | Continuous verification + artifact-bound, no-skip automated qualification before RC1; product polish, diagnostics, executable documentation, and public-path proof closure through v0.59.4–v0.59.13; optional extended soaks are supplemental only; v0.56 additionally proven by a `SimRuntime` mixed-version scenario |

Sixty-nine versions at ~6 person-weeks each is the full path from an empty
