...
| Pre-v1 Product Experience & Quality | v0.59.8–v0.59.17 | Single-source product surface manifest, simple executable documentation, golden-path project templates (local/Kafka/CDC Compose), structured diagnostics across all surfaces, public-path scenario/differential framework, full lifecycle/client/backend test closure, SQL semantics and type completeness, production lifecycle and health contracts, supported deployment profiles, and capacity/support guidance. |
| v1.0 Release | after v0.59.17 | The exact signed candidate artifacts pass every bounded automated qualification gate with zero open P0/P1 defects; optional extended soaks may supplement, but do not replace or block, the automated evidence; `v1.0.0` is then tagged. |
...
| v0.59.5 | Runtime Introspection & Operator Clarity | Add `SHOW ROCKSTREAM CAPABILITIES` backed by embedded runtime capability registry (`OBS-01`); add `SELECT rockstream_version()` returning `CandidateIdentity` semantic version over pgwire (`OBS-02`); enrich `SHOW VIEW STATUS` with runtime frontier, checkpoint, state size, spill, recovery, and recommended action fields (`OBS-03`); add read-only `rockstream_catalog` system tables for `nodes`, `sources`, `views`, `checkpoints`, and `capabilities` (`CAT-01`). | `SHOW ROCKSTREAM CAPABILITIES` and `rockstream_catalog.capabilities` agree with embedded `capabilities.toml` registry; `rockstream_version()` matches candidate semantic version over pgwire; `SHOW VIEW STATUS` and `rockstream_catalog.views` return full runtime status and progress; all five catalog tables are read-only, bounded, and redact sensitive credentials. | Unit, LFS, MinIO, TC |
...
| v0.59.7 | Error-Reference Generation & Contract Closure | Create structured error catalog and generator for `RS-XXXX` reference documentation and Rust constants (`DOC-01`); add conformance checks and drift gates; run final pre-v1 documentation and dispatch-wiring conformance sweep across all new SQL, SHOW, and catalog surfaces. | Error catalog, generated Rust constants, and generated error documentation agree with zero drift; every public `RS-XXXX` code has structured descriptor metadata, SQLSTATE mapping, and next-step text; dispatch-wiring check confirms zero missing paths across all new SQL, SHOW, and catalog surfaces. | Unit |
| v0.59.8 | Single-Source Product Surface | Add `rockstream-docgen`, normalized `ProductSurfaceManifest` (`DOC-001`), CLI/config/function/catalog/metric/error contributors, deterministic generated JSON, and drift gates (`DOC-004` foundation). | Manifest is deterministic; all public IDs resolve; generated output is clean; mutations to any public registry fail the drift gate. | Unit |
| v0.59.9 | Current, Simple, Executable Documentation | Reorganize docs by user persona (`DOC-002`), shorten README, separate history, generate references (`DOC-004`), add executable snippets/transcripts (`DOC-003`), and validate links and terminology. | README and getting-started paths execute; generated references match manifest; old public links resolve; no current doc claims unsupported behavior. | Unit, LFS, TC |
| v0.59.10 | Golden Path Complete | Add `rockstream init` (`GP-001`–`GP-006`), local/Kafka/PostgreSQL-CDC templates, maintained Compose profiles, verifier and cleanup services, canonical examples, and a reference application. | Every template runs from a clean directory, produces exact maintained-view results, fails clearly under common setup errors, cleans up, and runs again. | Unit, LFS, MinIO, Postgres, Kafka, TC |
| v0.59.11 | Structured Diagnostics Everywhere | Create canonical descriptor/diagnostic types (`ERR-001`–`ERR-008`), stable keys, retry classes, SQLSTATE validation, correlation IDs, redaction, renderers, lookup surfaces, and migrate every public error path. | No manual user-facing error strings; pgwire/CLI/JSON/log/support outputs agree; every code is documented; redaction mutation tests pass. | Unit, LFS, TC |
...
| v0.59.13 | SQL Contract, Lifecycle, Client, Documentation & Backend Test Closure | Define the exact SQL semantics and system limits promised for v1 while closing the existing lifecycle, client, documentation, and backend-test program. Document numeric precision and scale, arithmetic overflow and casts, NULL propagation and three-valued logic, duplicate rows, unmatched DML, transaction visibility, timestamp precision/time zones/DST, identifier folding, collation and string ordering, text-key aggregate/join behavior, timestamp grouping/join/window-key behavior, prepared-statement array parameters and `ANY($1)`, and the explicit PostgreSQL compatibility boundaries; generate `docs/sql-semantics.md` and `docs/limits.md` from authoritative behavior and limit metadata where practical. Also add lifecycle race/restart/recovery coverage (`TST-007`–`TST-010`), external client workflows, real Kafka/PostgreSQL/MinIO paths, documentation/golden-path scenarios, and zero-hidden-skip enforcement. | Every admitted semantic has executable conformance coverage against the batch oracle or PostgreSQL reference; every published limit is sourced from a runtime/configuration constant or an explicitly measured bound; generated documents are drift-checked; exceeding each bound fails with its documented `RS-XXXX` code; the Core behavior ledger is complete across required levels/backends; all golden paths and current docs run; all public surfaces agree; and no required high-level test is skipped or retried. | Unit, LFS, MinIO, Postgres, Kafka, TC |
| v0.59.14 | Common SQL & Type Completeness | Implement and prove the practical ordinary type-operation matrix promised for v1: integers, Boolean, text, UUID, date, timestamp, timestamptz, decimal/numeric within documented bounds, and common array parameter forms. Close the remaining ordinary combinations for numeric/decimal arithmetic, overflow and casts, NULL-preserving expressions and three-valued logic, text-key aggregates and joins, timestamp grouping/joins/window keys, prepared-statement array parameters and `ANY($1)`, and exact collation/string-ordering behavior. Floating-point equality joins remain explicitly unsupported unless separately admitted with a proof obligation. | A type-by-operation matrix covers every admitted combination across expressions, filters, joins, grouping, windows, DML, prepared statements, and incremental maintenance; numeric overflow/cast behavior matches the v0.59.13 contract; array parameters round-trip through simple and extended protocols; text and temporal keys produce oracle-identical results; collation/order tests are deterministic; unsupported floating-point equality joins fail clearly with their documented error rather than producing an unverified result. | Unit, LFS, TC |
| v0.59.15 | Graceful Lifecycle, Shutdown & Health Contract | Define SIGTERM/SIGINT, crash, eviction, drain, and role-shutdown behavior; stop new work, finish or abort epochs safely, flush durable state, release leases, close clients, enforce a configurable deadline, and expose structured `/live`, `/ready`, and `/health` endpoints with startup, draining, dependency, and degraded-state semantics. | Binary-level lifecycle tests start a workload, terminate gateway/control/worker roles, and verify committed rows survive, leases move, subscriptions close cleanly, and the old process exits within its deadline; readiness transitions are asserted during startup, dependency loss, and drain, while health responses include role, version, dependencies, and actionable reasons. | Unit, LFS, MinIO, TC |
| v0.59.16 | Supported Platforms & Deployment Profiles | Publish supported CPU architectures, Linux distributions and libc expectations, OCI image and `docker run`/Compose profiles, non-root container behavior, ports, storage and object-store requirements, resource recommendations, upgrade-safe configuration, and reference systemd and Kubernetes deployment profiles with a minimal Helm chart. | Clean-machine runs pass for each supported profile; x86-64 and ARM64 binaries and multi-architecture images start the same smoke workload; container, systemd, and Kubernetes profiles run as non-root with documented persistence and networking; the support matrix identifies tested S3-compatible, PostgreSQL, and Kafka versions and rejects unsupported combinations clearly. | Unit, LFS, MinIO, Postgres, Kafka, TC |
| v0.59.17 | Capacity Planning, Schema Evolution & Support UX | Add measured small/medium/large sizing profiles and explain the drivers of state, checkpoint, spill, cache, and skew costs; publish executable schema-evolution cookbooks for compatible and incompatible source changes, dependent views, and backfills; integrate `rockstream support diagnose` with redacted support bundles, issue identifiers, detected dominant causes, and next steps. | Each sizing profile publishes throughput, latency, RSS, state, spill, checkpoint, and object-store assumptions from reproducible workloads; schema cookbook procedures run end to end and fail closed for incompatible changes; `support diagnose` emits a bounded, redacted artifact whose findings match induced stale-view, lag, and dependency failures, and issue templates request that artifact. | Unit, LFS, MinIO, Postgres, Kafka, TC |

After v0.59.17, create `v1.0.0-rc.1` from one protected source SHA and freeze its
...
| v0.23–v0.59.17 | Continuous `formal-verify` + path-coupling (DC.1–DC.2); pre-release relaxed-bounds sweep (DC.4) | all `.fizz` specs | all M1–M7 | A coordination-protocol change without a model touch fails CI; the automated qualification suite re-runs the relaxed-bounds sweep against the candidate SHA |
...
| Phase 17 — Production Readiness & 1.0 Finalization | v0.53 – v0.59.17 (including the v0.53.1–v0.59.17 sub-versions added by the 2026-08-12, 2026-08-18, and 2026-08-19 reviews) | Continuous verification + artifact-bound, no-skip automated qualification before RC1; product polish, diagnostics, executable documentation, public-path proof closure through v0.59.13, SQL type-completeness proof at v0.59.14, and the lifecycle, deployment, and capacity work through v0.59.17; optional extended soaks are supplemental only; v0.56 additionally proven by a `SimRuntime` mixed-version scenario |

Seventy-three versions at ~6 person-weeks each is the full path from an empty
