...
| v1.0 Release | after v0.59.3 | The exact signed candidate artifacts pass every bounded automated qualification gate with zero open P0/P1 defects; optional extended soaks may supplement, but do not replace or block, the automated evidence; `v1.0.0` is then tagged. |
...
After v0.59.3, create `v1.0.0-rc.1` from one protected source SHA and freeze its
toolchain, lockfile, build container, artifacts, deployment configuration,
workload, fault policy, and thresholds. Run the complete v0.59.2 qualification
...
| v0.23–v0.59.3 | Continuous `formal-verify` + path-coupling (DC.1–DC.2); pre-release relaxed-bounds sweep (DC.4) | all `.fizz` specs | all M1–M7 | A coordination-protocol change without a model touch fails CI; the v0.59.2 automated qualification suite re-runs the relaxed-bounds sweep against the candidate SHA |
...
| Phase 17 — Production Readiness & 1.0 Finalization | v0.53 – v0.59.3 (including the v0.53.1–v0.59.3 sub-versions added by the 2026-08-12 and 2026-08-18 reviews) | Continuous verification + artifact-bound, no-skip automated qualification before RC1; optional extended soaks are supplemental only; v0.56 additionally proven by a `SimRuntime` mixed-version scenario |
...
connector quarantine, a deliberate reduction of the external connector surface to two sources and one sink (v0.52.3–v0.52.5), an operator-grade CLI and arrangement debugger, explainable freshness, internal mTLS and secrets management, a proven rolling-upgrade/disaster-recovery story, and a written, machine-checked v1 contract establish the v0.59 technical preview. Evidence integrity, true end-to-end automated qualification, reproducible release engineering, security provenance, and contract reconciliation then close out 1.0 through v0.59.1–v0.59.3 before the signed RC and final tag.
